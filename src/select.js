...
selectedAtt((el) => {
	return el.matches(
		`${optionsSelector} > ${optionSelector}, ${addAttribute(
			containerSelector,
			">" + optionSelector
		)}`
	);
...
document.addEventListener("input", function (e) {
	...
	if (e.target.matches(containerSelector)) {
		...
		if (e.target.save) e.target.save();
	}
});
...
document.addEventListener("click", function (e) {
	...
	for (let [el, instance] of container) {
		...
		} else if (el.classList.contains("open")) instance.close();
	}
});
...
CoCreateSelect.prototype = {
	...
	init: function (selectContainer) {
		if (container.has(selectContainer)) return;

		this.selectContainer = selectContainer;

		this.selectedContainer = selectContainer.querySelector(
		...
		this.input = selectContainer.querySelector(
			` :scope > ${inputSelector}`
		);
		...
		if (this.input) {
			this.input.addEventListener("keydown", function (e) {
				...
				if (tags == "true" && keyCode == 13 && this.value.length > 0) {
					...
					let option = selectedToOption.get(
		...
		if (selectContainer.read) selectContainer.read();

		selectContainer.getValue = () => {
			let element = this.selectContainer;
			let value = Array.from(element.selectedOptions).map((selOption) =>
				selectedToOption.has(selOption)
					? selectedToOption.get(selOption).getAttribute("value")
					: ""
			);
			if (value.length <= 1) value = value.length <= 1 ? value[0] : value;
			value = value ? value : "";
			return value;
		};
		...
		selectContainer.setValue = (value) => {
			...
			else if (value) instance.selectOption(value);
			else instance.unselectAll(false);
		};
	},
	...
	open: function () {
		...
		this.selectContainer.dispatchEvent(
			new CustomEvent("CoCreateSelect-open")
		);
	},
	...
	close: function () {
		...
		this.selectContainer.dispatchEvent(
			new CustomEvent("CoCreateSelect-close")
		);
	},
	...
	getOptionCounterpart: function (optionStr) {
		return this.selectContainer.querySelector(
			`${optionSelector.trim()}[value='${optionStr}']`
		);
	},
	...
	validateSelect: function () {
		...
		} else if (this.selectedContainer.children.length) this.unselectAll();
	},

	selectOption: function (
		option,
		closeOnMultiple = true,
		innerText,
		doEvent = true
	) {
		if (!option) return;
		...
		if (typeof option == "string") {
			...
			if (optionC)
				...
				let template = this.selectContainer.querySelector(
					".template, [template]"
				);
				let optionsContainer =
					this.selectContainer.querySelector("[array]");
				...
				for (let el of els) {
					el.setAttribute("object", option);
					let array = el.getAttribute("array");
					if (array == "{{object.array}}") {
						el.setAttribute("array", optionsCollection);
					}
				}
			} else {
			...
			if (template) {
				...
				if (value) {
					...
					let opt = this.selectContainer.querySelector(
						`[value='${value}']`
					);
	...
	unselectOption: function (option, doEvent = true) {
		...
		value = decodeURIComponent(value);
		let selectedOption = this.selectedContainer.querySelector(
			`[value='${value}']`
		);
		if (selectedOption) {
	...
	__fireSelectedEvent: function (detail) {
		...
		this.selectContainer.dispatchEvent(changeEvent);
	}
};
